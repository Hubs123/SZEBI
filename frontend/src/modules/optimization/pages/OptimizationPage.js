...
const OptimizationPage = () => {
    ...
    const handleToggleStatus = async (plan) => {
        try {
            ...
            await optimizationApi[action === 'run' ? 'runPlan' : 'stopPlan'](plan.id, userId);
            loadPlans();
        } catch (err) {
    ...
    return (
        <div className="panel">
            <div className="simulation-header">
                ...
                {view !== 'menu' && (
                    <button className="btn" onClick={() => setView('menu')}>⬅ Powrót do menu</button>
                )}
            </div>
            ...
            {view === 'menu' && (
                <div className="dashboard-stats" style={{ marginTop: '2rem' }}>
                    <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => setView('generate')}>
                        <h3>➕ Wygeneruj nowy plan</h3>
                        <p>Wybierz parametry i stwórz nową strategię.</p>
                    </div>
                    <div className="stat-card" style={{ cursor: 'pointer' }} onClick={() => setView('table')}>
            ...
            {view === 'generate' && (
                <div className="results" style={{ padding: '2rem', textAlign: 'center' }}>
                    ...
                    <br />
                    <button className="btn" onClick={handleGenerate}>Potwierdź i Generuj</button>
                </div>
            )}
            ...
            {view === 'table' && (
                <div className="simulation-table-container">
                    <table className="simulation-table">
                        ...
                        {plans.map(plan => (
                            <tr key={plan.id}>
                                ...
                                <td className="value-cell">{plan.costSavings.toFixed(2)} PLN</td>
                                <td>
                                        <span className={`battery-level ${plan.status === 'Active' ? 'high' : 'low'}`}>
                                            {plan.status}
                                        </span>
                                </td>
                                <td>
                    ...
                    {plans.length === 0 && <div className="no-data-message">Brak dostępnych planów.</div>}
                </div>
            )}
        </div>
    );
